...
- **公网:http://110.184.175.58:38383/**
- 内网:http://10.5.10.95:38383/(部署与运维走这个)

> 注:公网 80 端口目前被其他服务占用(返回 503),所以公网访问需带 `:38383` 端口。若想 `http://110.184.175.58/` 直达,需在服务器上把 80 的反向代理指到 38383。
...
## 部署(内网操作:dashuai@10.5.10.95,公网通过路由端口映射暴露 38383)
...
- 已做:scrypt 加盐哈希、HttpOnly/SameSite Cookie、SQL 全参数化、请求体排空(防 keep-alive 失步)、原子切换(防并发竞态)、连接超时、输入校验、前端输出转义、登录耗时对齐(防用户名枚举)
- **未做,公网长期运行建议尽快补**:HTTPS(现在是明文 HTTP,密码在公网裸奔)、验证码/限流(可被脚本刷注册灌水)、CSRF token、内容审核
- 种子账号密码 `sanshuai` 是公开的,公网上任何人都能登这些账号发内容——玩梗可以,别放重要数据
- 数据库记得定期备份:`~/dalanying/dalanying.db`
